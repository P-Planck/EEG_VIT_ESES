...
def process_eeg_data(base_input_path, base_output_path, num_folders, start_idx, csv_file, num_categories):
    ...
    for folder_idx in tqdm(range(start_idx, num_folders + 1), desc="Processing folders"):
        ...
        if not os.path.exists(edf_file) or not os.path.exists(excel_file):
            print(f"Skipping folder {folder_idx}: Missing EDF or Excel file.")
            continue
        ...
        try:
            ...
            yy_folder = os.path.join(output_folder_path, 'yy')
            nn_folder = os.path.join(output_folder_path, 'nn')
            yn_folder = os.path.join(output_folder_path, 'yn')
            ny_folder = os.path.join(output_folder_path, 'ny')
            os.makedirs(yy_folder, exist_ok=True)
            os.makedirs(nn_folder, exist_ok=True)
            os.makedirs(yn_folder, exist_ok=True)
            ...
            if folder_idx in grouped_data.groups:
                ...
                for _, row in tqdm(folder_data.iterrows(), desc=f"Processing samples in folder {folder_idx}", total=len(folder_data), leave=False):
                    ...
                    # Determine the folder based on label and CSV result
                    label = labels[img_idx - 1]
                    if label == 1 and csv_value == 1:
                        folder = yy_folder
                    elif label == 0 and csv_value == 0:
                        folder = nn_folder
                    elif label == 1 and csv_value == 0:
                        folder = yn_folder
                    elif label == 0 and csv_value == 1:
                        folder = ny_folder
                    else:
                        print(f"Unexpected case: label={label}, csv_value={csv_value} for folder {folder_idx}, image {img_idx}")
                        continue

                    img.save(os.path.join(folder, f'image_{img_idx}.jpg'))

        except Exception as e:
            print(f"Error processing folder {folder_idx}: {e}")

# Define parameters
base_input_path = './EEG_data/EEG_IMG_BASE'
base_output_path = './EEG_data/EEG_IMG_sinv'
num_folders = 30
start_idx = 1
